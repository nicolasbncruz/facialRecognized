...
class Database {
	...
	public final String Database_pass = "mysqlsql";
	...
	public boolean init() throws SQLException {
		try {
			...
			try {
				Properties properties = getConfigurationDb();
				this.con = DriverManager.getConnection(
						getUrlConnection(properties),
						getStringToObject(properties.get("database.user")),
						getStringToObject(properties.get("database.password")));
	...
	private String getUrlConnection(Properties properties) {
		return "jdbc:mysql://" + getStringToObject(properties.get("database.host")) + ":" +
				getStringToObject(properties.get("database.port")) + "/" +
				getStringToObject(properties.get("database.name"));
	}

	private String getStringToObject(Object object) {
		return String.valueOf(object);
	}

	private static Properties getConfigurationDb() throws Exception {
		if (properties == null) {
			System.out.println("new connection");
			InputStream input = new FileInputStream("src/db/db_config.properties");
